...
The API also can have its behaviour changed setting the
macro \monoespaco{W\_FORCE\_LANDSCAPE}. If we have an environment
...
Notice that in fact we have two variables to store position $x$ and
...
variables \monoespaco{\_x}, \monoespaco{\_y}
...
and \monoespaco{rotation} store the values with the rotated axis. When the
...
The list of default variables and attributes passed to the shader also
must be part of the source code. We will define them later, but for
now we show that they will be in this variable:
